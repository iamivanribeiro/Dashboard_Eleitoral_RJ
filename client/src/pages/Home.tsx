...
export default function Home() {
  ...
  const municipioSugestoes = useMemo(() => municipios.map((m) => m.nome).sort((a, b) => a.localeCompare(b, "pt-BR")), []);
  ...
  const kpis = useMemo(() => {
    const mediaSinergia = municipiosFiltrados.length === 0 ? 0 : municipiosFiltrados.reduce((acc, item) => acc + item.sinergia, 0) / municipiosFiltrados.length;
    const totalVotos = municipiosFiltrados.reduce((acc, item) => acc + (candidato === "flavio" ? item.votosFlavio : item.votosCanella), 0);
  ...
  const filtrosAtivos = [
    ...
    sinergiaMin > 0
      ...
      : null,
    populacaoMin > 0
      ? { key: "populacao", label: `População ≥ ${populacaoMin.toLocaleString("pt-BR")}`, clear: () => setPopulacaoMin(0) }
      : null,
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100">
      <a href="#etapa-recorte" className="sr-only focus:not-sr-only focus:fixed focus:top-2 focus:left-2 focus:z-50 bg-white border rounded px-3 py-2 text-sm">Ir para recorte</a>
      <a href="#etapa-mapa" className="sr-only focus:not-sr-only focus:fixed focus:top-14 focus:left-2 focus:z-50 bg-white border rounded px-3 py-2 text-sm">Ir para mapa</a>
      <a href="#etapa-comparacao" className="sr-only focus:not-sr-only focus:fixed focus:top-26 focus:left-2 focus:z-50 bg-white border rounded px-3 py-2 text-sm">Ir para comparação</a>
      ...
      <header className="border-b border-slate-200 bg-white/90 backdrop-blur supports-[backdrop-filter]:bg-white/80 sticky top-0 z-20">
        <div className="max-w-7xl mx-auto px-4 py-4 md:py-5 flex items-center justify-between gap-3">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-slate-900 tracking-tight">Dashboard Eleitoral RJ 2026</h1>
            <p className="text-slate-600 mt-1 text-sm md:text-base">Fluxo de decisão: recorte → exploração → comparação → validação</p>
          </div>
          <Sheet>
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 md:py-8 space-y-6">
        <Card className="p-3 md:p-4 border-slate-200 bg-white/80 backdrop-blur-sm">
          <div className="flex items-center gap-2 text-sm font-medium text-slate-700 mb-2"><Compass className="h-4 w-4" /> Navegação por contexto</div>
        ...
        <section id="etapa-mapa" className="space-y-3 scroll-mt-28" aria-label="Etapa 2: exploração geográfica">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold text-slate-900">Etapa 2 · Explore no mapa</h2>
            <p className="text-xs text-slate-500">Clique em um município para abrir o detalhamento</p>
          </div>
        ...
        <section id="etapa-comparacao" className="hidden lg:grid grid-cols-1 lg:grid-cols-2 gap-6 scroll-mt-28" aria-label="Etapa 3: comparação e validação">
          <div className="space-y-3">
            <h2 className="text-lg font-semibold text-slate-900">Etapa 3 · Compare por município</h2>
            <Card className="p-4 border-slate-200">
              <p className="text-xs text-slate-500 mb-2">Últimos municípios vistos</p>
              {ultimosMunicipios.length === 0 ? (
                <p className="text-sm text-gray-500">Ainda não há histórico de seleção.</p>
              ) : (
          ...
          <div className="space-y-6" id="comparativo-regional">
            <div className="space-y-3">
              <h2 className="text-lg font-semibold text-slate-900">Etapa 3 · Compare por região</h2>
